...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    ...
    let  handel: thread::JoinHandle<_> = thread::spawn(move || {
        numbers.into_iter().map(|x: i32| x * 2 ).collect()
    });
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let handle1 = thread::spawn(move || a.into_iter().sum());
    let handle2 = thread::spawn(move || b.into_iter().sum());
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    thread::scope(|s| {
        let handle1 = s.spawn(|| a.into_iter().sum::<i32>());
        let handle2 = s.spawn(|| b.into_iter().sum::<i32>()); 
        (handle1.join().unwrap(), handle2.join().unwrap())
    })
}
