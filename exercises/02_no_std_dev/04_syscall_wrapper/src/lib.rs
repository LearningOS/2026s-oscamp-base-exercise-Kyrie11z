...
pub unsafe fn syscall3(id: usize, arg0: usize, arg1: usize, arg2: usize) -> isize {
    ...
    core::arch::asm!(
        "syscall",
        inlateout("rax") id => ret,
        in("rdi") arg0,             // 参数 1 放进 rdi
        in("rsi") arg1,             // 参数 2 放进 rsi
        in("rdx") arg2,             // 参数 3 放进 rdx
        out("rcx") _,               // 告诉编译器：rcx 寄存器的值被破坏了，别依赖它
        out("r11") _,               // r11 也被破坏了
...
pub unsafe fn syscall3(id: usize, arg0: usize, arg1: usize, arg2: usize) -> isize {
    ...
    core::arch::asm!(
        "svc #0",
        in("x8") id,                 // aarch64 用 x8 传递 syscall 号
        inlateout("x0") arg0 => ret, // x0 既用来传第一个参数，也用来接返回值
        in("x1") arg1,
        in("x2") arg2,
        options(nostack, preserves_flags)
    );
...
pub fn sys_write(fd: usize, buf: &[u8]) -> isize {
    unsafe {
        syscall3(NATIVE_SYS_WRITE, fd, buf.as_ptr() as usize, buf.len() as usize)
    }
}
